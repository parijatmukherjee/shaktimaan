...
# Resolve an explicit SHAKTIMAAN_INIT_DIR project override (the directory that
# *contains* .shaktimaan/), for non-interactive / CI use — e.g. running a Shaktimaan
# command against a member project from a monorepo root without cd.
#
# Precondition: SHAKTIMAAN_INIT_DIR is non-empty. Echoes the validated absolute
# project root, or prints an error and returns 1. Strict by design: the path
# must exist and contain .shaktimaan/, with no silent fallback to cwd or the
...
resolve_shaktimaan_init_dir() {
    ...
    if ! init_root="$(CDPATH="" cd -- "$SHAKTIMAAN_INIT_DIR" 2>/dev/null && pwd)"; then
        echo "ERROR: SHAKTIMAAN_INIT_DIR does not point to an existing directory: $SHAKTIMAAN_INIT_DIR" >&2
        return 1
    fi
    if [[ ! -d "$init_root/.shaktimaan" ]]; then
        echo "ERROR: SHAKTIMAAN_INIT_DIR is not a Shaktimaan project (no .shaktimaan/ directory): $init_root" >&2
        return 1
    fi
    printf '%s\n' "$init_root"
}
...
# Safely read .shaktimaan/feature.json's "feature_directory" value.
...
read_feature_json_feature_directory() {
    local repo_root="$1"
    local fj="$repo_root/.shaktimaan/feature.json"
...
# Persist a feature_directory value to .shaktimaan/feature.json.
...
_persist_feature_json() {
    ...
    # Ensure .shaktimaan/ directory exists
    mkdir -p "$repo_root/.shaktimaan"
...
get_feature_paths() {
    ...
    # --no-persist so pure path resolution never writes .shaktimaan/feature.json,
    ...
    # Split decl/assignment so a SHAKTIMAAN_INIT_DIR validation failure in
...
get_invoke_separator() {
    ...
    local integration_json="$repo_root/.shaktimaan/integration.json"
    local separator="."
    local parsed=0
...
#   1. .shaktimaan/templates/overrides/
#   2. .shaktimaan/presets/<preset-id>/templates/ (sorted by priority from .registry)
#   3. .shaktimaan/extensions/<ext-id>/templates/
#   4. .shaktimaan/templates/ (core)
resolve_template() {
    local template_name="$1"
    local repo_root="$2"
    local base="$repo_root/.shaktimaan/templates"
    ...
    local presets_dir="$repo_root/.shaktimaan/presets"
...
" 2>/dev/null); then
    ...
    local ext_dir="$repo_root/.shaktimaan/extensions"
    if [ -d "$ext_dir" ]; then
