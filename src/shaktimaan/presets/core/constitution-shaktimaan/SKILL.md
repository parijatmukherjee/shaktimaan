...
- After completing the constitution update, include a `Next Actions` section for each deferred
  ...
  as `/specify-shaktimaan`, without invoking it.
...
You are updating the project constitution at `.shaktimaan/memory/constitution.md`. The active
...
1. Run `.shaktimaan/scripts/bash/resolve-template.sh constitution-template --json` from the repository root and parse `TEMPLATE_CONTENT` as the active template.
   ...
   - If it fails, stop and report the resolution error; do not continue with only one contributing
     template layer.
   - If `.shaktimaan/memory/constitution.md` exists, load it as the source of current project-specific
...
6. Write the completed constitution back to `.shaktimaan/memory/constitution.md` (overwrite).
...
Write only `.shaktimaan/memory/constitution.md`; do not create or modify template source files.
