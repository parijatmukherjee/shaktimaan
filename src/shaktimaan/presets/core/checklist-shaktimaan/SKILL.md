...
- `checklists/requirements.md` is a separate built-in spec-quality checklist maintained by `/specify-shaktimaan` and `/clarify-shaktimaan`; do not treat that exception as applying to custom checklists generated here.
...
1. **Setup**: Run `.shaktimaan/scripts/bash/check-prerequisites.sh --json --template checklist-template` from repo root and parse JSON for FEATURE_DIR, AVAILABLE_DOCS list, and TEMPLATE_CONTENT.
...
**Important**: Each `/checklist-shaktimaan` command invocation uses a short, descriptive checklist filename and either creates a new file or appends to an existing one. This allows:
